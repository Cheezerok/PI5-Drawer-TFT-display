...
private:
    ...
    void setup_x11() {
        ...
        if (!x_display) {
            std::cerr << "Не удалось подключиться к X серверу\n";
            return;
        }
    ...
    void mouse_event_handler() {
        ...
        if (mouse_fd < 0) {
            std::cerr << "Не удалось открыть устройство мыши\n";
            return;
        }
        ...
        const int DOUBLE_CLICK_TIME = 300; // миллисекунды
        ...
        while (mouse_thread_running) {
            if (read(mouse_fd, data, sizeof(data)) == sizeof(data)) {
                ...
                // Обработка скролла
                if (data[0] & 0x8) { // Скролл вверх
                    change_brush_size();
                } else if (data[0] & 0x10) { // Скролл вниз
                    change_brush_size();
                }

                // Масштабирование координат мыши под размер дисплея
                int16_t new_x = cursor_x + x;
                int16_t new_y = cursor_y - y;

                // Ограничиваем координаты размерами дисплея
                ...
                // Обработка двойного клика левой кнопкой
                if (left_button) {
...
public:
    ...
    void run() {
        ...
        while (running) {
            // Обновляем зеркальное отображение
